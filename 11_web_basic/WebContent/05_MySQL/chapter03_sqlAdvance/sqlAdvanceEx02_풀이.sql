...
FROM
		MEMBER_TB M
             JOIN ORDER_TB O
			   ON M.MEMBER_ID = O.MEMBER_ID
              AND M.RESIDENCE = '서울' AND M.SEX = 'F'
GROUP BY
		M.RESIDENCE,
        M.SEX;
...
FROM
		MEMBER_TB M
			 JOIN ORDER_TB O
               ON M.MEMBER_ID = O.MEMBER_ID
              AND M.RESIDENCE IN ('서울', '경기', '인천') AND M.SEX = 'F'
GROUP BY
		M.RESIDENCE,
        M.SEX;
...
SELECT
		P.PRODUCT_CD,
        P.PRODUCT_NM,
        P.PRICE * O.ORDER_GOODS_QTY AS TOTAL_PRICE
FROM
		ORDER_TB O
			JOIN PRODUCT_TB P
              ON O.PRODUCT_CD = P.PRODUCT_CD
