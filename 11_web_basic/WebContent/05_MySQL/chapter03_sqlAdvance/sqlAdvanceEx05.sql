...
SELECT MEMBER_ID
FROM   MEMBER
WHERE  SEX = 'f' AND AGE BETWEEN 30 AND 39;
...
WHERE  CAR_CD NOT IN (SELECT DISTINCT CAR_CD
					  FROM   RENT);
...
# 가장 비싼 차량3개의 가격을 10%할인하기 (테이블명의 중첩될 경우 AS로 테이블명 지정)
SELECT   CAR_CD
FROM     CAR
ORDER BY PRICE DESC
LIMIT    3;
...
UPDATE CAR
SET    PRICE = PRICE*0.9
WHERE  CAR_CD IN ( SELECT TEMP_CAR_CD
				   FROM   (SELECT   CAR_CD
						   FROM     CAR
						   ORDER BY PRICE DESC
						   LIMIT    3) AS TEMP);
...
SELECT ROUND(AVG(TEMP.RENT_PERIOD))
FROM   (SELECT *
		FROM   RENT
		WHERE  CAR_CD IN (SELECT CAR_CD
						  FROM   CAR
						  WHERE  BRAND_NM = '기아')) AS TEMP;
