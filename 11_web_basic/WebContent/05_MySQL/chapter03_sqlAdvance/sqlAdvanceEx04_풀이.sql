...
SELECT C.CAR_NM, AVG(R.RENT_PERIOD)
FROM   RENT R
	   JOIN CAR C
         ON R.CAR_CD = C.CAR_CD
	    AND C.CAR_NM = '아반떼';
...
# 6) '유저1'의 렌트 횟수 조회하기
SELECT R.MEMBER_ID, COUNT(*) AS RENTCNT
FROM   RENT R
WHERE  R.MEMBER_ID = 'USER1';
...
# 8) 20대 여성의 렌트일 조회하기
SELECT M.MEMBER_ID, R.RENT_PERIOD
FROM   RENT R
	   JOIN MEMBER M
         ON R.MEMBER_ID= M.MEMBER_ID
        AND M.SEX='F' 
...
SELECT substring(C.REG_DT, 1, 4), COUNT(*)
FROM   RENT R
	   JOIN CAR C
         ON R.CAR_CD = C.CAR_CD
GROUP BY
		substring(C.REG_DT, 1, 4);
