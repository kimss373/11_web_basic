...
SELECT PRODUCT_CD
FROM   (SELECT PRODUCT_CD, SUM(ORDER_GOODS_QTY)
		FROM   ORDER_TB
		WHERE  ORDER_DT BETWEEN '2020-01-01' AND '2020-12-31'
		GROUP BY PRODUCT_CD
        HAVING SUM(ORDER_GOODS_QTY) < 5) AS TEMP;
...
SELECT MEMBER_ID, SUM(ORDER_GOODS_QTY)
FROM   ORDER_TB
GROUP BY MEMBER_ID
HAVING SUM(ORDER_GOODS_QTY) > (SELECT SUM(ORDER_GOODS_QTY)
							   FROM   ORDER_TB
							   WHERE  MEMBER_ID = 'USER4');
