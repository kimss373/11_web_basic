/*
	...
	# 연습환경 셋팅작업
		...
		> Mysqlworkbench에서 새로고침 클릭 departments
        > employees 스키마를 확인
        > 끝
...
SELECT
		EMP_NO, SALARY, SALARY*0.033, SALARY*0.967
FROM
		SALARIES;
...
FROM
		EMPLOYEES
WHERE
		LAST_NAME LIKE '%o%' AND LAST_NAME NOT LIKE '%o';
...
# 36) 사원의 번호와 이름(성,이름)을 조회하기. (성,titles이름 모두 알파벳 순서) (EMPLOYEES)
SELECT
		EMP_NO, FIRST_NAME, LAST_NAME
FROM
		EMPLOYEES
