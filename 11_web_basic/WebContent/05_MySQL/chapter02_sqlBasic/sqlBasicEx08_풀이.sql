...
SELECT
		TITLE, PRICE, PUBLISHING_COM, SALES_CNT
FROM
		BOOK
ORDER BY
		SALES_CNT DESC
...
SELECT
		PRICE, PRICE*0.1 BGS, PRICE*1.1 PBGS
FROM
		BOOK;

		
# 13) 가격 , 할인률 , 가격에 할인률을 적용한 컬럼을 조회하기
SELECT PRICE, DISCOUNT_RATE, PRICE*(100-DISCOUNT_RATE)/100 RESULT
FROM BOOK;
		
# 14) 가격 , 할인률 , 10%부가세 , 가격에 할인률을 적용하고 원가의 10%를 더한 책의 전체 컬럼을 조회하기
SELECT
		PRICE, DISCOUNT_RATE, PRICE*0.1 VAT10, PRICE*(100-DISCOUNT_RATE)/100+(PRICE*0.1) RESULT
FROM
		BOOK;
...
DELETE FROM
		BOOK
WHERE
		TITLE='컴퓨터활용능력' OR TITLE='초등학생이 알아야할 숫자';
